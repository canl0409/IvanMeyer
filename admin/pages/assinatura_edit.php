...
if(isset($_GET['pid'])){  
$pd =  $p->getRegistro('assinaturas','assinaturaId',$_GET['pid']);
$alunoid = $pd['alunoId'];
} 
...
<input type="hidden"  value="1" name="editassinatura"    >
...
<div class="form-group ">
  ...
  <div class="col-lg-10">
    <select name="planoId" id="" class="form-control ">
      ...
      <option <?php   if($pd['planoId'] == $cu['planoId']){echo "selected";}  ?> value="<?=$cu['planoId'] ?>"><?=$cu['nome'] ?></option>
      <?php } ?>
    </select>
  </div>
</div>
...
<div class="col-lg-10">
<input type="text" readonly  minlength="2" name="vencimento" id="expira" value=" <?=date( 'Y-m-d', strtotime ($pd['vencimento']) );?>"  class="form-control default-date-picker ">
</div>
</div>
...
<div class=" col-lg-2">
<a href="?alunos_edit2&pid=<?=$alunoid?>&dAssId=<?=$pd['assinaturaId']?>" ><button type="button" class="btn btn-warning">Excluir</button></a>
</div>
</div>
</form>
</div>
