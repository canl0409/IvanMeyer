  <?php 
    ...
    if(isset($_POST['upid'])){ 
      ...
      $dados['upid'] =  $_POST['upid'];

      if(!empty($_FILES['capa']['name'])){
        $dados['capa'] =  $p->upload($_FILES['capa'],'../arquivos/capas');  
      }

      $add =  $p->updateRegistro($dados,'arquivos','id');   
    }


  if(isset($_GET['pid'])){  
    $pd =  $p->getRegistro('arquivos','id',$_GET['pid']);
  } 
...
<div class=" form">
<form action="?livros_edit&pid=<?=$_GET['pid']?>" method="POST" id="commentForm" class="cmxform form-horizontal tasi-form"    enctype="multipart/form-data">
...
<?php  if($pd['capa']){ ?>
<img src="../arquivos/capas/<?=$pd['capa']?>" alt="" style="max-width: 200px; max-height: 150px; line-height: 20px;">
<?php }else{  ?>
...
<div class="col-lg-10">
<input type="text" value="<?=$pd['titulo']?>" required="" minlength="2" name="titulo" id="titulo"   class="form-control ">
</div>
</div>
...
<div class="col-lg-10">
<textarea required="" minlength="2" name="link" id="link"   class="form-control "><?=$pd['link']?></textarea>
</div>
</div>
...
<div class="col-lg-10">
<input type="text" value="<?=$pd['senha']?>" minlength="2" name="senha" id="senha"   class="form-control ">
</div>
</div>
